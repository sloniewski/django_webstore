...
class TestCartShowsSummary(FunctionalTest):
    ...
    def test_cart_shows_summary(self):
        ...
        url = self.live_server_url + '/cart/list-item/'
        ...
        cart_items_list = [x.text for x in self.browser.find_elements_by_class_name('cart-list-item')]
        self.assertIn(prod_a.name, cart_items_list)
        self.assertIn(prod_b.name, cart_items_list)

        cart_summary = self.browser.find_element_by_id('cart-total-value')
